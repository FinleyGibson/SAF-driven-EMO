...
% subcaption package, requires redefining the caption style
\usepackage{subcaption}
\usepackage{textcase}
\DeclareCaptionTextFormat{up}{\newline \MakeTextUppercase{#1}}
\captionsetup[table]{justification=centering, textformat=up, font=small}
...
%As shown in Algorithm \ref{alg:BO}, t
 ...
 %REPEATED BLOCK Since evaluation of the acquisition function entails evaluating the model
...
%This penalty was originally calculated as the product of the $\nobj$ dimensional distances between the model prediction  $\hat{\bff}(\bx')$ at a location $\bx'$ summed over each point in approximation to the Pareto front: $\sum_{\bx_p\in\Papprox}\prod_{m=1}^{\nobj}|\fhat_m(\bx')-f_m(\bx_p)|$
%\mnotejf{Shouldn't this be $\hat{\bff}(\bx)$, to be in keeping with the notation for mono-surrogates?}\fnote{I think i have fixed, this. Do you think it needs its own equation? I dont think it really warrants it, but the $\sum_{\bx_p \in \Papprox}$ looks alittel clunky.}, but was later modified  to use the maximum over $\Papprox$ in order to reduce discontinuities in the resulting infill  \cite{wagner2010expected}. 
...
\begin{algorithm}[t!]
 ...
 \For{$j=1\xrightarrow{}\nobj$ \do}{
    ...
    $\Fapprox \gets \nondom(\data)$ \algremark{Approximate Pareto front}
    \eIf{exploitative}{ 
        $\bx_t \gets \underset{\mathbf{x}\in \parameterspace}{\argmax} \:
        \saf(\bmu(\mathbf{x}), \Fapprox, \theta_{1:\nobj})$ 
        \algremark{\safmu} 
        \label{alg:opt-SAFmu}
    } {
    $\bx_t \gets \underset{\mathbf{x}\in \parameterspace}{\argmax} \: \ei(\mathbf{x}, \Fapprox, \theta_{1:\nobj})$
...
\begin{table}[t]
    ...
    \caption{Walking Fish Group functions, showing the 18
      $(\nobj,\ndim)$ combinations tested.}\label{tab: f_table}
    \setlength{\tabcolsep}{2pt}
    \sisetup{table-format=1.2e-1,table-number-alignment=center}
    \resizebox{\columnwidth}{!}{%
        \begin{tabular}{|l|l|l|l|}
            ...
            & Comment\\
            \hline
            WFG1 & $(2,3), (3,4), (4,5)$ & $[0, 2d]^d$ &  Separable, uni-modal \\
            WFG2 & $(2,6), (3,6), (4,10)$ & $[0, 2d]^d$ &  Non-separable, multi-modal\footnotemark{}, $\paretofront$ is discontinuous. \\
            WFG3 & $(2,6), (3,6), (4,10)$ & $[0, 2d]^d$ &  Non-separable, uni-modal \\
            WFG4 & $(2,6), (3,8), (4,8)$ & $[0, 2d]^d$ &  Separable, multi-modal\\
            WFG5 & $(2,6), (3,8), (4,10)$& $[0, 2d]^d$ &  Separable, deceptive\\
            WFG6 & $(2,10), (3,6), (4,12)$ & $[0, 2d]^d$ &  Non-separable, uni-modal\\
            \hline
        \end{tabular}}
\end{table}
\footnotetext{Multi-modal in only the first objective $f_{1}$, uni-modal in $f_{2} \cdots f_{\nobj}$.}
...
\caption{Convergence %of tested algorithms 
...
% We further show that in multi-objective Bayesian optimisation there is no need for deliberately exploratory evaluations of the objective function, and that exploitative evaluation based on surrogate mean prediction yields better solutions in most cases. This presents an opportunity to broaden the scope of surrogate models used in multi-objective Bayesian optimisation to those which may be better able to model complex objective functions, but do not provide uncertainty estimates, such as support vector machines or random forests.
