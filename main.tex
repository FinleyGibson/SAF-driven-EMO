...
\end{equation}
The acquisition function can be cheaply optimised using methods such as an evolutionary algorithm (EA) without the need for repeated evaluation of the expensive objective function. Several broadly applicable acquisition functions have been specified, typically based on expected improvement or probability of improvement (PI) measures \cite{jones1998efficient,shahriari2015taking} and \EI has been shown to converge at a near optimal rate \cite{bull2011convergence}. This process of fitting a surrogate, and optimisation of the acquisition function is repeated,  to expand $\mathcal{D}$, iteratively until a good solution is found and some stopping criteria is satisfied or the computational budget is exhausted.
...
The image of $\mP$ under $\bff$ is the Pareto front. Typically in multi-objective optimisation the best we can aspire to is an \emph{approximation} this, as represented by a set of mutually non-dominating solutions. The line/surface in objective space which separates the area/volume dominated by any element of this set from that which is non-dominated is known as the summary attainment front (SAF).
...
Bayesian approaches to solving EMOPs\mnote{this has been defined as meaning efficient MOP -- not sure it makes sense in here?} tend to use Gaussian Process surrogates and largely differ in the acquisition functions by which they select the parameters at which to next evaluate the true objective function.
...
We propose using a signed \maximin distance from the attainment front, similar to that used by Svenson and Santer \citep{svenson2016multiobjective}, as the infill criterion for a multi-surrogate assisted EMO optimisation process with GP surrogates as proposed by Emmerich \etal \cite{emmerich2006single}. We will forgo the popular \EI computation in favour of mean prediction in order as explored by De Ath \etal \cite{death2019greed}.



pseudocode here Algorithm  \ref{alg:ourAlg_pseudocde}.\mnote{incomplete}
...
The chosen objective functions on which to benchmark the different strategies are a subset the Walking Fish Group problems \citep{huband2005scalable} WFG1-WFG6, designed to build on the well-known DTLZ problem set \citep{deb2005scalable} by including deceptive functions and functions where the parameters dictating distance from, and position along the Pareto front are not separable. The dimensionality of the parameter-space $d$ for the WFG problems is scalable, as are the number of objectives $M$.\mnote{in section 2.1 $M$ is the number of initial samples, and $n$ not $d$ is the domain size. } We tested each function over three configurations of each problem in WFG1-WFG6, with 2, 3 and 4 objectives, and parameter spaces ranging from 6-12 dimensions. The exception to this is WFG2, which was poorly optimised by all optimisers, and so had the parameter space reduced to the minimum permissible $d=[3, 4, 5]$ to aid convergence. A full table of the test functions can be seen in Table \ref{tab: f_table}.
...
The results of the optimisations, as measured by \igd are summarised in table \ref{tab: igd_results}.\mnote{why no hypervolume table? you say earlier both measures are used} The performances of the optimiser's are, as we may expect, somewhat problem dependant. After the full 150 budgeted evaluations of the objective functions, \safmu either had the lowest median \igd score, or did not differ significantly from the optimiser with the best score in 9 of the 18 tested problems, this is one more than the state-of-the art \smsego, but is equalled by the exploitative adaption \smsegomu. As we might expect, when measured by the $\mathcal{S}$-metric measure the results are somewhat more favourable to \smsego and \smsegomu, with \safmu only being equivalent to the best performer on 9 optimisations, as opposed to \smsegomu's 9. 
...
It should be noted that all optimiser performed poorly on WFG2, with the majority of the Pareto Front left unexplored. \parego performed relatively well compared to the other optimisers, but many of the surrogate models were beaten by the \lhs benchmark here. It is worth noting that WFG2 is the only problem here with a discontinuous Pareto front, and in addition the manifold on which the Pareto front lies\mnote{do you mean front or set here?} extends into the hypervolume dominated by the Pareto front, meaning much of the dominated objective space is unattainable at each step, which is a feature unique to this function from those tested here but could be a factor in the poorer performance by optimisers which attempt to infill this space.\mnote{this is a bit confusing -- isn't this to do with the shape generator function, rather than the problem per se?}
