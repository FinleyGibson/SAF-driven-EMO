...
\section{Exploitative SAF (\MSAF)}\label{section:our_method}
\begin{itemize}
...
In our proposed \MSAF  method, the expected maximin fitness improvement function proposed by Svenson \citep{svenson2016multiobjective} is adapted to an infill criterion calculated as the signed maximin distance, centred at zero, from the summary attainment front. 
...
\begin{itemize}
    ...
    \item why d6? reduced dimensionality for wfg1 to facilitate finding better solutiuons
    measurements taken using dominated hypervolume and igd+
    \item igd+ samples taken by oversampling the pareto front, adn then drawing a random sample from this using the attianment samlping techinique described in richards paper. 
    \item results compared by their median value, with statistical equivlance testedl using Holm-bonferoni corrrected wilcoxen signed rank test. 
    
    \item hypothesis one: SAF just as good
        \begin{ite}
            \item Saf better than SE
        \end{ite}
...
\begin{table}[t]
    ...
    \resizebox{0.5\textwidth}{!}{%
        \begin{tabular}{|l|r|l|r|l|}
            \hline
            Test function &  Obj. & Domain & $d$ & Comment\\
            \hline
            WFG1 & 2, 3, 4 & $[0, 2d]^d$ & $5$ &  XXX \\
            WFG2 & 2, 3, 4 & $[0, 2d]^d$ & $5$ & multi-modal, discontinuous Pareto front, non-separable\\
            WFG3 & 2, 3, 4 & $[0, 2d]^d$ & $5$ &  XXX \\
            WFG4 & 2, 3, 4 & $[0, 2d]^d$ & $5$ & multi-modal, separable\\
            WFG5 & 2, 3, 4 & $[0, 2d]^d$ & $5$ & separable, non-mixed parameters\\
            WFG6 & 2, 3, 4 & $[0, 2d]^d$ & $5$ & non-separable, non-mixed parameters\\
            \hline
        \end{tabular}}
...
\section{Discussion}
\begin{enumerate}
    \item, \MSAF, SMSEGO and SMS-EGO-mu potentially resample already chosen points
